...
from config import OPENAI_COMPLETION_OPTIONS
...
class ChatGPT:
    def __init__(self, model="gpt-3.5-turbo"):
        assert model in {"text-davinci-003",
                         ...
                         "gpt-4-turbo"}, f"Unknown model: {model}"
    ...
    async def send_message_stream(self, message, dialog_messages=[], chat_mode="assistant"):
        ...
        while answer is None:
            try:
                if self.model in {"gpt-3.5-turbo-16k", "gpt-3.5-turbo", "gpt-4", "gpt-4-1106-preview", "gpt-4-turbo"}:
                    messages = self._generate_prompt_messages(message, dialog_messages, chat_mode)
    ...
    async def send_vision_message(
        ...
        while answer is None:
            try:
                if self.model in {"gpt-4-vision-preview", "gpt-4-turbo"}:
    ...
    #
    # def improve_transcribe_audio(self, audio_file, chat_mode, model) -> str:
